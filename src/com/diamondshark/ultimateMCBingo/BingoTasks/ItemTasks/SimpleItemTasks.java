...
public class SimpleItemTasks extends AbstractTask {
    ...
    public SimpleItemTasks(int ItemIndex)
    {
        ...
        playerItemQuantity = new int[ITEM_TASK_LIST[ItemIndex].getItemType().length];
    ...
    public boolean checkCompletion(Player player)
    {
        ...
        int newStatus = AbstractTask.NOT_STARTED;

        int totalItems = 0;

        for(int i = 0; i < playerItemQuantity.length; i++)
        {
            totalItems += playerItemQuantity[i];
        }

        if(totalItems >= (ItemQuantity * playerItemQuantity.length))
        {
            newStatus = AbstractTask.PENDING;
        }
        else if(totalItems > 0)
        {
            newStatus = AbstractTask.IN_PROGRESS;
        }

        boolean hasChanged = super.getStatus() != newStatus;
    ...
    public void checkInventory(Player player)
    {
        ...
        {
            playerItemQuantity[i] = 0;
            for (ItemStack item : player.getInventory().getContents()) {
                if (item != null && item.getType() == ITEM_TASK_LIST[ItemIndex].getItemType()[i]) {
                    playerItemQuantity[i] += item.getAmount();
                }
            }
        }
    }
